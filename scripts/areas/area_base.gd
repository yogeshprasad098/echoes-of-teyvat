...
func _check_subviewport_fallback() -> void:
	var sub_container: Node = get_node_or_null("%SubViewportContainer")
	var fallback: Node = get_node_or_null("%FallbackSprite")
	if sub_container == null or fallback == null:
		return
	var sub_viewport: SubViewport = get_node_or_null("%SubViewport")
