...
func _run() -> void:
	...
	_expect(packed != null, "Boss 1 arena scene loads")
	if packed == null:
		_finish()
		return
...
func _check_environment_nodes(root: Node) -> void:
	_expect(root.get_node_or_null("ArenaEnvironment") == null, "Boss 1 arena removes boss-specific environment art")
	...
	_expect(bg != null and bg.sprite_frames.resource_path.ends_with("ember_fields_clean_ambient_bg_sprite_frames.tres"), "Boss 1 uses level 1-5 clean animated background")
	var ground := root.get_node_or_null("Ground") as TileMapLayer
	_expect(ground != null and ground.tile_set.resource_path.ends_with("ember_fields_clean_preview_tileset.tres"), "Boss 1 uses level 1-5 clean tileset")
...
func _finish() -> void:
	if _failures.is_empty():
		print("RESULT: PASS Boss 1 arena environment check")
		quit(0)
		return
	for failure in _failures:
		push_error(failure)
	print("RESULT: FAIL %d Boss 1 arena environment item(s)" % _failures.size())
	quit(1)
