...
func _check_environment_nodes(root: Node) -> void:
	_expect(root.get_node_or_null("ArenaEnvironment") == null, "Boss 2 arena removes boss-specific environment art")
	...
	_expect(bg != null and bg.sprite_frames.resource_path.ends_with("ember_fields_clean_ambient_bg_sprite_frames.tres"), "Boss 2 uses level 1-5 clean animated background")
	var ground := root.get_node_or_null("Ground") as TileMapLayer
	_expect(ground != null and ground.tile_set.resource_path.ends_with("ember_fields_clean_preview_tileset.tres"), "Boss 2 uses level 1-5 clean tileset")
