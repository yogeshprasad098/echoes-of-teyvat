...
extends EnemyBase
## Storm Peaks elite caster. Stays grounded on platform routes, telegraphs,
## then fires map-native electro attacks.

enum State { IDLE, CHASE, CAST, DEAD }
enum AttackKind { ORB, LINE }
...
const ELECTRIC_ORB_SCENE := preload("res://scenes/projectiles/enemy_electric_orb.tscn")

const MAX_ATTACK_RANGE: float = PhysicsModel.TILE_SIZE_PX * 9.0
const VERTICAL_TOLERANCE: float = PhysicsModel.TILE_SIZE_PX * 2.4
const ATTACK_COOLDOWN: float = 1.65
const CAST_WINDUP: float = 0.62
const CAST_RECOVERY: float = 0.35
const DEATH_CLEANUP_DELAY: float = 0.9
const LINE_DAMAGE: float = 14.0
const LINE_HIT_WIDTH: float = PhysicsModel.TILE_SIZE_PX * 7.0
const LINE_HIT_HEIGHT: float = PhysicsModel.TILE_SIZE_PX * 1.4

var _state: State = State.IDLE
var _target: CharacterBase = null
var _facing: int = 1
var _attack_cooldown: float = 0.55
...
func _ready() -> void:
	super._ready()
	max_health = maxf(max_health, 95.0)
...
func _start_cast() -> void:
	...
	_attack_index += 1
	_current_attack = AttackKind.LINE if _attack_index % 3 == 0 else AttackKind.ORB
	_face_target()
...
func _release_attack() -> void:
	...
	if sprite.sprite_frames and sprite.sprite_frames.has_animation(&"attack"):
		sprite.play(&"attack")
	if _current_attack == AttackKind.LINE:
		_release_lightning_line()
	else:
		_release_electric_orb()
	_attack_cooldown = ATTACK_COOLDOWN

func _release_electric_orb() -> void:
	var orb := ELECTRIC_ORB_SCENE.instantiate() as EnemyElectricOrb
...
func _release_lightning_line() -> void:
...
func _strike_lightning_line(midpoint: Vector2, flip: bool) -> void:
	...
	if is_instance_valid(_target):
		...
		if absf(delta_to_target.x) <= LINE_HIT_WIDTH * 0.5 and absf(delta_to_target.y) <= LINE_HIT_HEIGHT * 0.5:
			_target.take_damage(LINE_DAMAGE)
			ENEMY_VFX.spawn(get_parent(), ELITE_VFX_FRAMES, &"electric_burst", _target.global_position + Vector2(0, -12), false, Vector2.ONE, 12)
